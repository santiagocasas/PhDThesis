...
\beql{eq:moreGeneral-Gaussian-Likelihood}
...
\begin{equation}\label{eq:dataCovariancePk}
...
\inserteq{L=\frac{1}{(2\pi)^{m/2} \det (C_{k_{i} k_{j}}) } \exp \left[-\frac{1}{2}\sum_{i}^{m} 
\frac{\delta^2_{k_i}}{C_{k_{i} k_{j}}} \right] }{eq:likelihood-Pofk}
...
\beql{eq:Fisher-fromloglik-second}
...
spectrum:
\beql{eq:Fisher-fromloglik-inPk}
...
\beql{eq:Fisher-MatrixForm-GaussianGeneral}
...
In \cite{Khedekar, Majumdar} have also compared directly the Fisher matrix formalism with a direct MCMC approach
...
\subsubsection{Galaxy Clustering (FF)}

The Fisher matrix for the galaxy power spectrum has the following
form \cite[see][]{seo2007improved, amendola2012testing}:
...
\subsection{The observed power spectrum 2}
...
\subsubsection{Observed Power (FF)}
...
\section{The Fisher Matrix Code \comm{this one is ok but not a full manual}}
\begin{itemize}
