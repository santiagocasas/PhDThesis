...
perihelion shift of Mercury there was no experimental need for it. It was more than a decade later, thanks to observations
...
One century later, General Relativity (GR) has passed numerous very stringent tests; from laboratory experiments \cite{dimopoulos2007testing} , to
low orbit \cite{iorio2003impact} and solar system tests \cite{bertotti_test_2003}, to constraints from pulsar timing \cite{kramer2006tests} and the recent
exciting first detection of gravitational waves by the LIGO/VIRGO collaboration \cite{abbott2016observation}.
It is impressive that a theory that was formulated on the grounds of some very fundamental
...
Despite the successes of standard GR, there is one extra ingredient that has to be added to explain the observations at the largest scales of the Universe.
To account for the accelerated expansion of the Universe, which was observationally verified almost 20 years ago,
(\cite{riess1998observational,perlmutter1999measurements}), Einstein's General Relativity needs to be supplemented with a Cosmological Constant (known as CC or $\Lambda$).
Even taking into account the fact that $\Lambda$ is allowed by the classical theory, as was proven by Lovelock in the 1970's \cite{lovelock1971einstein} (see \cref{sec:GR-framework}) and 
it fits well present cosmological observations \cite{planck_collaboration_planck_2016-2}, it
possesses many unsatisfactory properties from the theoretical point of view. 
We will review the problems associated to the Cosmological Constant in \cref{sub:CC-problem} below.

A popular alternative to the cosmological constant is to introduce an extra dynamical degree of freedom, which is able to explain the present acceleration of the Universe and its apparent dominance at late cosmological times. Many of these models can also lead 
...
Here, $R=R^\mu_\mu$ is the Ricci scalar and $R^\munu$ is the Riemann tensor (see standard GR textbooks like the \citet{wald2010general}, for a thorough definition of these terms), 
both of them which are functions of derivatives of the metric up to second order. The volume element is defined as $\textrm{d}x^4 \sqrt{-g}$, where
...
As it was famously expressed by \citet{misner1973gravitation}:
"Matter tells space-time how to curve and geometry tells matter how to move" .

Another important property of these field equations, is that due to a geometric property called the Bianchi identity, which
states that the covariant divergence of the Einstein tensor is identically zero, 
$\nabla_{\mu} G^\munu = 0$, 
we can ensure that the energy-momentum tensor is covariantly conserved:
\beeqc$
\nabla_{\mu} T^\munu = 0
$
therefore, locally, one recovers all the properties of classical and fluid mechanics.
...
Thanks to a theorem by Vermeil and Cartan (\cite{pesic2003weyl}) 
and further simplified by Lovelock (\cite{lovelock1971einstein}) we can state that in 4 dimensions,
...
The FLRW metric \cref{eq:FLRW-full} due to its scale factor, which is dependent on time, implies immediately that the Universe has dynamics
...
In this model, and according to the latest observations (see \cite{planck_collaboration_planck_2016-2}),
...
Since the Universe is expanding and we measure astrophysical objects using only electromagnetic radiation (except after 2016 
when Gravitational Wave Astronomy was born \cite{abbott2016observation}), it is important to define certain distances that can be measured in cosmological observations.
Light follows null-geodesics, so that under an FLRW metric as in \cref{eq:FLRW-full}, 
null-like particles satisfy the equation:
\beeqc$
...
\done\todo{Review this according to Javi}
...
would yield $\langle p_{\rm vac} \rangle / \langle \rho_{\rm vac} \rangle  = 1/3$, instead of the expected $w=-1$ (see \cite{martin_everything_2012}).
...
50 orders of magnitude larger than $\rho_{\Lambda}$ (see \cite{martin_everything_2012}).
In the absence of a powerful symmetry able to cancel vacuum fluctuations, such as supersymmetry \cite{weinberg1982cosmological} or scale-invariance \cite{barrow2005exact}, this unnatural fine tuning has to be performed order by order in perturbation theory.
This instability under radiative corrections, is the core of the Cosmological Constant problem 
\cite{burgess2004quantum, martin_everything_2012}.


\subsubsection*{The coincidence problem \label{subsub:coinc-problem}}
...
Dark Energy models with tracker and scaling solutions (see \cite{amendola_dark_2010}) in which $\rho_{DE}$ catches the trend of $\rho_{m}$ no matter which initial conditions have been chosen
...
structure formation, for example through backreaction effects \cite{buchert2012backreaction}. 
However, these models have not been able to fully explain the observations so far.
...
Other scientists resort to the anthropic principle \cite{weinberg1987anthropic, smolin2007scientific}
...
can only have certain precise range of values \cite{weinberg1989cosmological, weinberg1987anthropic}. While it is true that these constraints are applicable
to our case, this explanation is not widely accepted as a "physical" solution of the problem.


\subsection{ Linearized Einstein Equations \label{sub:linearized-Einstein-eqs}}

Since we observe non-homogeneities in the Universe around us, we need to go beyond
the background description of General Relativity given by the Friedmann equations \ref{eq:Friedmann-1st}-\ref{eq:Friedmann-2nd}.
...
ds^{2} &= a^2(\tau) \left(-(1+2\Psi)d\tau^{2} + (B_{,i} + S_i)d\tau dx^i \right. \\
       & \left. -  (-2\Phi \delta_{ij} + 2E_{,ij} + F_{i,j} + F_{j,i} + h_{ij})  dx^i dx^j \right)
$
...
to gravitational wave detection \cite{abbott2016observation} and B-mode polarization in the CMB \cite{ade2015joint},
...
agree with the gauge invariant ``Bardeen`` potentials, since in this gauge: $ E = B = 0$ ( see \citet{mukhanov2005physical}).
