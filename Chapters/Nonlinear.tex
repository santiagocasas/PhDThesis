...
\footnote{This chapter is based on a publication in preparation by: 
...
\todo{Implement Florians comments}
...
\todo{add short derivation, Florians comments, Schwinger Dyson, SPT, explain
1-loop terms shortly}
...
shown in \cref{fig:change-fsigma8-Y}, where we are using the data points from \mcite{Macaulay et al. (2013)}. \todo{Valerias comments, comment on data}
\todo{Improve labels and legs of figs}
...
\end{figure}
Finally, we show in \cref{fig:change-PS-resummed-YB1}, for the same Horndeski model YB1, its non-linear power spectrum vs. the one in $\lcdm$,
...
%\footnote{The results of this chapter so far, will be published in a paper together with Dr. Massimo Pietroni and
