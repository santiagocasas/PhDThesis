...
If we want to maximize over the parameter $\theta_m$, i.e. we want to obtain the errors on all other parameters,
when $\theta_m$ is fixed, then we just need to remove from the Fisher matrix, the rows and the columns
corresponding to that parameter $m$:
\beeq$
\tilde F = M_{(n-1) \times n}^{T} F_{n \times n} M_{n \times (n-1)} 
$
where $M_{n \times (n-1)$ 
