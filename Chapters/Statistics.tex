\chapter{Statistics in Cosmology \label{chap:Statistics}} % Main chapter title
...
In cosmology, we are interested in studying observables which are influenced by a large number of random processes, giving them a stochastic nature.
...
Using the Central Limit Theorem \cite{marinucci2011random}, we can show that the sum of a large number of random variables, that originate from independent random processes, will yield a variable that it is almost exactly Gaussian distributed. The Gaussian probability distribution $p(x)$ for a random variable $x$, with mean $\mu$ and variance $\sigma$ is given by
\begin{equation}
p(x) = \frac{1}{\sqrt{2 \pi \sigma^2}}\exp\left( \frac{(x-\mu)^2}{2\sigma^2} \right)
\end{equation}
As we will see, Gaussian distributed variables will be very important in observations and analysis of cosmological observables.
...
its statistical properties must be homogeneous and isotropic too \cite{peacock_cosmological_1999-2}.
...
in cosmology we postulate the ergodicity principle \cite{adler2010geometry}, which states that for sufficient large volumes
...
where the following recursion relations (obtained by Isserli's or Wick's theorem \cite{bernardeau_large-scale_2001}) are valid:
\begin{align}
...
\xi(\vec{r}) = \langle \delta (\vec{y}-\vec{r})  \delta (\vec{y}) \rangle 
= \frac{1}{V} \int \delta (\vec{y}-\vec{r})  \delta (\vec{y}) \dx{3}{y} \quad.
\end{equation}
...
two-point correlation function and will not be statistically isotropic.
An example of this can be seen in Baryon Acoustic oscillations \cite{bassett_baryon_2009}, in which the correlation function has different values, depending if it is
...
\begin{equation}
\delta(\vec k) = \frac{1}{V} \int \mathrm{d}^3 x \; \delta(\vec x) \exp(-i \vec{k} \cdot \vec{x})
\end{equation}
and its inverse is
\begin{equation}
\delta(\vec x) = \frac{V}{(2\pi^3)} \int \mathrm{d}^3 k \; \delta(\vec k) \exp(i \vec{k} \cdot \vec{x}) \quad ,
\end{equation}
where $V$ is the volume and both $\delta$ functions in Fourier and real space are dimensionless.
...
\int \mathrm{d}^3 x \, \mathrm{d}^3 y \langle \delta (\vec{x})  \delta (\vec{y}) \rangle \exp(-i \vec{k}_1 \cdot \vec{x} + i \vec{k}_2 \cdot \vec{y}) )
 \quad .
\end{equation}
...
\end{equation}
The relation between the expected value of $\langle \delta (\vec{k}_1)  \delta^{*} (\vec{k}_2) \rangle$ and the power spectrum $P(\vec k)$ can be obtained by a similar straightforward calculation \cite{dodelson2003modern, ,amendola_dark_2010}, where taking into account
that the $\delta$ field is real-valued, one can obtain:
\begin{equation}
\langle \delta (\vec{k})  \delta (\vec{k}') \rangle = (2\pi)^3  P(k) \delta_{D}(\vec{k}+\vec{k}') \quad .
\end{equation}
...
the Fourier modes $\vec{k}$ and $\vec{k}'$ for the density fluctuations are mutually uncorrelated.
...
\done\todo{add Kevins comments}
The subject of Bayesian statistics is a very complex, but very important topic in modern
statistics, data science and the physical sciences \cite{gregory2005bayesian}
...
Then what physicists are interested in, is in constraining the model parameters $\theta_i$ and try to find
...
and $P(D)$ is the evidence (the probability of the data), which for our purposes we can ignore, since it
represents just a normalization factor in our applications.
Also here, a discussion on the significance and the effects of the prior is out of our scope \cite{amendola_dark_2010, ,dodelson2003modern}. However, in our case it will be an efficient way of combining results from previous
...
\begin{itemize}
	...
	are in general different to the frequentist ones, if we have used a prior.
	...
	If $\alpha=0.683,0.954,0.997$, these regions are called 1, 2 and 3$\sigma$ regions respectively.
...
more than 7 or 8 parameters. So, techniques like Markov Chain Monte Carlo (MCMC) (see \cite{Dunkley2005} for a review)
have become more and more sophisticated. In these techniques, the basic idea is to explore the parameter
space using a random walk, which is itself guided by the steepness or flatness of the likelihood at that point.
In this way there are much less evaluations in places where the likelihood is flat and many evaluations
where the likelihood changes very rapidly. Two very well known
codes in the cosmological community are \textsc{MontePython} and \textsc{CosmoMC} \cite{lewis2002cosmological,audren2013conservative}
which are used to evaluate the likelihood of many large and modern experiments like \textit{Planck} \cite{planck_collaboration_planck_2016-2}.

Many approximations can be made in order to estimate the likelihood or at least 
to find its maximum. 
...
For (approximately) Gaussian distributed parameters, the parameter covariance matrix will be of great
...
But what if we don't have data available yet? For example when we want to know 
with which accuracy a certain future experiment is going to be able to constrain certain parameters of
a model. 
This is where the Fisher formalism \cite{hu_weak_1999,seo_baryonic_2005} enters.
...
$\hat \theta$, or equivalently where the $\chi^2$ has a minimum:
\begin{equation}
...
is an indication of how fast the likelihood changes around the peak \cite{dodelson2003modern}. 
...
formal field of study called Information Geometry \cite{arwini2008information}}.
...
In this section we follow closely the argumentations of \cite{dodelson2003modern} and \cite{amendola_dark_2010}.

We are interested in finding the distribution of matter in the Universe, and as we have seen before,
for sufficiently large scales, the overdensity of matter at a specific scale $k$ is given by $\delta(k)$.
...
\langle\delta_{k_i} \delta^*_{k_j} \rangle = C_{k_{i} k_{j}} = \delta_{ij} \left( P(k_i, z) + \frac{1}{\bar{n}(z+\Delta z)} \right)
$
The reader can find the full details of the calculation in \cite{dodelson2003modern}.
...
of the data measurements $\bm \mu$ is not zero. (see \cite{tegmark1997karhunen}
...
\right]
$
This last term, the expected value of the derivatives of the data matrix $\bm D$, is usually zero 
...
dropped and higher order ``Fisher tensors`` are used to approximate the true underlying likelihood \cite{sellentin_breaking_2014}.
Also recently, there has been some work in extending the simplifying assumptions of 
diagonal covariance, redshift-bin correlations and $k$-space correlations \cite{bailoni_improving_2016,blot_matter_2014}.

In \cite{khedekar_cosmology_2012} the authors also compared directly the Fisher matrix formalism with a direct MCMC approach
...
are obtained by direct measurements in the sky and end-to-end simulations \cite{laureijs_euclid_2011}.
...
in \cref{eq:obs-power-basic}. It can be shown \cite{ross_information_2015} that the 
...
This assumption breaks at very large and very small scales \cite{desjacques_large-scale_2016}.
...
For more details on those topics see \cite{taruya_baryon_2010,scoccimarro_redshift-space_2004, Bonvin:2014owa}.
...
For more details and derivations, see the comprehensive review by \cite{bartelmann_weak_2001}.
...
$P_{\kappa \kappa}$ (see \cite{amendola_dark_2010, hu_weak_1999}),
...
the maximum $k$ at which we trust the power spectrum; see also \cite{casas_fitting_2016}).
...
is a diagonal ``cutoff'' matrix, discussed in \cite{casas_fitting_2016} whose entries increase to very high
...
some of which are listed in \cref{tab:GC-specifications} and \cref{tab:WL-specifications} and the rest can be found in the above cited references.
...
\begin{table}[h]
	...
	\begin{tabular}{|c|cccc|c|}
		...
		\hline 
	\end{tabular}\caption[Specifications for future GC surveys.]
         {Specifications for
		the spectroscopic
...
\end{equation}
We can then define a Kullback-Leibler matrix $\mathcal{K}$, introduced in (\cite{casas_linear_2017} appendix F) composed of the KL-divergence measure among our observables, defined as:
...
The following discussion was introduced in \cite{casas_fitting_2016} in reference to
...
\done\todo[Maybe]{Add derivation of this equation}
\begin{equation}
...
implementation of the  \fito code, which has been used in the projects 
...
for several projects in the field of Fisher matrix forecasts and perturbation theory \cite{amendola_model-independent_2013}.
...
or package.
The supported codes are the Boltzmann codes: \texttt{CAMB} \cite{lewis_efficient_2000} and \texttt{CLASS} \cite{lesgourgues_cosmic_2011-2,lesgourgues_cosmic_2011-3}.
The fitting functions from Eisenstein \& Hu \cite{eisenstein_power_1999}, \texttt{Halofit} \cite{smith_stable_2003} and the \texttt{Cosmic Emulator}
...
called \texttt{COPTER} \cite{upadhye_large-scale_2014}.
...
Let us first consider the derivatives of the observed power spectrum, \cref{eq:obs-power-basic},
...
This method is called the "BAO" method \cite{seo_improved_2007}, since in BAO observations
...
the same fiducial parameters and the same ordering, we just need to add the Fisher matrices together: $F_{A+B} = F_A + F_B$ and from $F_{A+B}$ we can obtain the combined contours and errors. However, there is a caveat: this can only be done if the experiments $A$ and $B$ are statistically independent from each other. Otherwise, we have to take into account cross-correlations \cite{mohammad_group-galaxy_2015}.
...
for example the \textsc{DALI} code \cite{Sellentin:2015axa}, which allows
...
the effect of changing the fiducial value of the parameters on the forecasted errors on these parameters. Recently \cite{reischke_variations_2016}
...
In our approach, which is still a work in progress,
