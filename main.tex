...
This thesis deals with the relevance of non-linear structure formation in the Universe,
for the determination of the cosmological parameters beyond the standard $\lcdm$ model.
Future galaxy surveys will be able to measure $\sim 10^7$ galaxies and will determine with high
precision the galaxy and matter power spectrum. This function encodes substantial 
information on the cosmological model, especially at non-linear scales.
In this dissertation we want to investigate how to make use of non-linearities 
in structure formation in order to discriminate between models of dark energy and modified gravity that go beyond the standard cosmological $\lcdm$ model. Here, we approach this task from three different
angles. In the first one we use N-body simulations to produce observables that can be constrained with future observations, as in the case of Coupled Dark Energy and Growing Neutrino Models. 
In the second, we use semi-analytic methods for the non-linearities 
and we study its impact on parameterizations of Modified Gravity which are independent of a specific model. Finally, we concentrate on specific cases of Horndeski's theory to compute non-linear corrections
to the matter power spectrum using renormalization methods in perturbation theory.
For the parameter constraints, we focus on Galaxy Clustering and Weak Lensing observables. 
To forecast how well future surveys will be able to measure the parameters entering Modified Gravity and
Dark Energy models, we employ the Fisher matrix formalism, for which we also develop a specialized
code.
